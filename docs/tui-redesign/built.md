...
- s12-ecr-policy-unavailable 608d1b6 — ECR's repository and lifecycle policy reads carry their errors on the row, so a throttled or denied read says unavailable on the Overview, the Config tab and the preview action instead of "none"; ECR's own not-found exceptions are the genuine absence
- s12-ec2-performance-unavailable 8aa46fa — the EC2 Network section's Performance row reads unavailable when the instance-type lookup failed, because every type carries a rating and "none" was a fact AWS never reported
- final-dod 3bd4ae7 — the terminal audit found no gap: 37 closed tickets set-equal to built.md and each reviewed, both gate commands green, all 31 benchmarks running, the layout invariants passing from an unmodified tree, all eight formatters covering empty/error/missing-optional, and 4/4 Playwright journeys on two runs
