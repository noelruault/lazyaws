...
func TestVPCOverviewOrdersItsCountLines(t *testing.T) {
	// The sort itself is asserted on countsByKey with six keys, because a rendered line long enough to hold six is cut to the pane and a Contains assertion cannot see a truncation.
	if got, want := countsByKey(map[string]int{"f": 6, "c": 3, "a": 1, "e": 5, "b": 2, "d": 4}), "a (1), b (2), c (3), d (4), e (5), f (6)"; got != want {
		t.Errorf("countsByKey() = %q, want %q", got, want)
	}
