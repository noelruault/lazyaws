...
func TestEKSCluster(t *testing.T) {
	...
	if cluster.Region != "eu-west-1" {
		t.Errorf("Expected region 'eu-west-1', got '%s'", cluster.Region)
	}
}
...
func TestUpgradeClusterVersionGuards(t *testing.T) {
	if err := (&Client{}).UpgradeClusterVersion(context.Background(), "my-cluster", "1.30"); err == nil {
		t.Error("UpgradeClusterVersion() with nil EKS client should error")
	}
	if err := (&Client{}).UpgradeClusterVersion(context.Background(), "", "1.30"); err == nil {
		t.Error("UpgradeClusterVersion() with empty cluster name should error")
	}
	if err := (&Client{}).UpgradeClusterVersion(context.Background(), "my-cluster", ""); err == nil {
		t.Error("UpgradeClusterVersion() with empty target version should error")
	}
}
